...
/**
 * Create device fingerprint from IP hash + User Agent hash for better unique device tracking.
 * This helps distinguish multiple devices behind the same NAT/proxy.
 * @param {string} ipHash - Hashed IP address
 * @param {string} userAgent - User-Agent string
 * @returns {string | null} Combined fingerprint hash or null
 */
function getDeviceFingerprint(ipHash, userAgent = "") {
  if (!ipHash) return null;
  if (!userAgent || typeof userAgent !== "string") return ipHash; // Fallback to IP hash only
  
  // Hash user agent (normalize to avoid minor variations)
  const uaNormalized = userAgent.toLowerCase().trim();
  const uaHash = crypto.createHmac("sha256", IP_HASH_SECRET).update(uaNormalized).digest("hex").substring(0, 16);
  
  // Combine IP hash + UA hash for unique device identification
...
module.exports = { hashIp, getDeviceFingerprint, getDeviceType, getBrowser, getOS };
