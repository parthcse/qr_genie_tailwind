...
export async function downloadDesignedQR(qrValue, designData = {}, format = "png", size = "default", filename = "qr-code") {
  ...
  try {
      ...
      if (hasFrame) {
        ...
        frameStyles = `
          .qr-frame {
            ...
            background: ${frameBg};
            ${designData?.frameBgUseGradient ? `background-image: ${frameBg};` : ""}
            padding: 16px;
    ...
    if (hasFrame) {
      ...
      // Frame Color always controls border color only (no gradient support)
      const frameBorderColor = frameDesignData?.frameColor || "#000000";
      ...
      // Use frameTextColor if available, otherwise fallback to frameColor
      const textColor = frameDesignData?.frameTextColor || frameBorderColor;
